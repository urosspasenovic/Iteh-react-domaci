  ...
  return (
    <div className="card" style={stil}>
      ...
      <div className="card-body">
        ...
        <Button className="btn" text ="Subscribe" onClick={() => onAdd(product.title, product.price, product.id)}></Button> </> )   
        :( <>  <p className="card-text-in-my-subscriptions">You are part of {product.title} family</p> 
          <p className="card-price">{product.price} <MdEuro /></p>
        <Button className="btn" text ="Unsubscribe" onClick={() => removeSubscription(product.id, product.price)}></Button> </> )         
        }
